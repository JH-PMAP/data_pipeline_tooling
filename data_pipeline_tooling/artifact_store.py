...
class AzureDataLakeClient:
    def __init__(
        ...
        self.file_system_client = self.get_or_create_file_system(file_system_name)
    ...
    def create_directory(self, directory_name):
        try:
            self.file_system_client.create_directory(directory_name)
        except Exception as e:
            raise e
    ...
    def upload_file(self, local_file_path, remote_file_path, mode="r"):
        try:
            ...
            file_client.append_data(data=contents, offset=0, length=len(contents))
    ...
    def ls(self, remote_dir):
        try:
            paths = self.file_system_client.get_paths(path=remote_dir)
            for path in paths:
                print(path.name)
        except Exception as e:
            raise e
    ...
    def folder_exists(self, remote_file_path):
        ...
        try:
            dir_client = self.file_system_client.get_directory_client(path)
    ...
    def recursive_copy(self, root, run_type, version_id, repo_name):
        ...
        for rel_root, directory, files in os.walk(root):
            for File in files:
                files_to_copy.append(os.path.join(rel_root, File))
        for file_path in files_to_copy:
            self.copy_file(
                file_path, f"{repo_name}/{run_type}/{version_id}/{file_path}"
            )


def generate_meta_data(version_id, run_type, job_id, run_id, repo_name, commit_hash):
